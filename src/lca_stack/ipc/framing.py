...
class FramedSocket:
    ...
    The payload is typically a serialized Protobuf message.
    """
    ...
    def read_frame(self) -> bytes:
        ...
        length = struct.unpack(">I", bytes(self._recv_buffer[:4]))[0]
    ...
    def read_message(self, msg: object) -> object:
        payload = self.read_frame()
        # Protobuf messages expose ParseFromString.
        parse = getattr(msg, "ParseFromString", None)
        if parse is None:
            raise TypeError("msg must be a protobuf message")
        parse(payload)
        return msg

    def write_message(self, msg: object) -> None:
        serialize = getattr(msg, "SerializeToString", None)
        if serialize is None:
            raise TypeError("msg must be a protobuf message")
        data: bytes = serialize()
